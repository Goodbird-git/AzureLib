...
public class GeoReplacedEntityRenderer<E extends Entity, T extends GeoAnimatable> extends EntityRenderer<E> implements GeoRenderer<T> {
	...
	public GeoReplacedEntityRenderer(EntityRenderDispatcher renderManager, GeoModel<T> model, T animatable) {
		super(renderManager);
	...
	public void actuallyRender(PoseStack poseStack, T animatable, BakedGeoModel model, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
		...
		if (shouldSit && this.currentEntity.getVehicle() instanceof LivingEntity) {
			lerpBodyRot = Mth.rotLerp(partialTick, ((LivingEntity) this.currentEntity.getVehicle()).yBodyRotO, ((LivingEntity) this.currentEntity.getVehicle()).yBodyRot);
		...
		float headPitch = Mth.lerp(partialTick, this.currentEntity.xRotO, this.currentEntity.xRot);
...
//		RenderSystem.setShaderTexture(0, getTextureLocation(animatable));
	...
	public void renderFinal(PoseStack poseStack, T animatable, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
		...
		if (this.currentEntity instanceof Mob) {
			Entity leashHolder = ((Mob) this.currentEntity).getLeashHolder();

			if (leashHolder != null)
				renderLeash(((Mob) this.currentEntity), partialTick, poseStack, bufferSource, leashHolder);
		}
	}
	...
	protected void applyRotations(T animatable, PoseStack poseStack, float ageInTicks, float rotationYaw, float partialTick) {
		...
		if (livingEntity != null && livingEntity.deathTime > 0) {
			...
			if (this.currentEntity instanceof Player) {
				if (!((Player) currentEntity).isModelPartShown(PlayerModelPart.CAPE))
					return;
			} else {
	...
	public int getPackedOverlay(T animatable, float u) {
		...
		return OverlayTexture.pack(OverlayTexture.u(u), OverlayTexture.v(((LivingEntity) this.currentEntity).hurtTime > 0 || ((LivingEntity) this.currentEntity).deathTime > 0));
	}
	...
	public <H extends Entity, M extends Mob> void renderLeash(M mob, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, H leashHolder) {
		double lerpBodyAngle = (Mth.lerp(partialTick, mob.yBodyRotO, mob.yBodyRot) * ((float) Math.PI / 180)) + 1.5707963267948966;
