...
public class GeoEntityRenderer<T extends Entity & GeoAnimatable> extends EntityRenderer<T> implements GeoRenderer<T> {
	...
	public GeoEntityRenderer(EntityRenderDispatcher renderManager, GeoModel<T> model) {
		super(renderManager);
	...
	public void actuallyRender(PoseStack poseStack, T animatable, BakedGeoModel model, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
		...
		if (!isReRender) {
			float headPitch = Mth.lerp(partialTick, animatable.xRotO, animatable.xRot);
	...
	protected void applyRotations(T animatable, PoseStack poseStack, float ageInTicks, float rotationYaw, float partialTick) {
		...
		if (livingEntity != null && livingEntity.deathTime > 0) {
			...
			if (animatable instanceof Player) {
				if (!((Player) animatable).isModelPartShown(PlayerModelPart.CAPE))
					return;
			} else {
