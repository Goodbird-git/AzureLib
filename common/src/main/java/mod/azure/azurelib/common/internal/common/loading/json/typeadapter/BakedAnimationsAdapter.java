...
public class BakedAnimationsAdapter implements JsonDeserializer<BakedAnimations> {
    ...
    private static Pair<String, JsonElement> getTripletObjBedrock(String timestamp, JsonObject keyframe) {
        if (keyframe.has("vector")) {
            ...
            JsonArray array = postElement.isJsonArray() ? postElement.getAsJsonArray() : GsonHelper.getAsJsonArray(
                    postElement.getAsJsonObject(), "vector");
            ...
            return Pair.of(
                    String.valueOf(NumberUtils.isCreatable(timestamp) ? Double.parseDouble(timestamp) : 0 + 0.0000001),
                    array);
        }
