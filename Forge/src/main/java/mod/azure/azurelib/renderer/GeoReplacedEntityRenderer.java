...
public class GeoReplacedEntityRenderer<E extends Entity, T extends GeoAnimatable> extends EntityRenderer<E> implements GeoRenderer<T> {
    ...
    public void actuallyRender(MatrixStack poseStack, T animatable, BakedGeoModel model, RenderType renderType, IRenderTypeBuffer bufferSource, IVertexBuilder buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        ...
        if (shouldSit && this.currentEntity.getVehicle() instanceof LivingEntity) {
            lerpBodyRot = MathHelper.rotLerp(partialTick, ((LivingEntity) currentEntity.getVehicle()).yBodyRotO,
                    ((LivingEntity) currentEntity.getVehicle()).yBodyRot);
        ...
        float headPitch = MathHelper.lerp(partialTick, this.currentEntity.xRotO, this.currentEntity.xRot);
...
//		RenderSystem.setShaderTexture(0, getTextureLocation(animatable));
     ...
     * Functionally equivalent to {@link LivingRenderer#getFlipDegrees LivingEntityRenderer.getFlipDegrees}
     */
    ...
    public boolean shouldShowName(E entity) {
        ...
        if (entity instanceof MobEntity && (!entity.shouldShowName() && (!entity.hasCustomName() || entity != this.entityRenderDispatcher.crosshairPickEntity)))
            return false;
        ...
        return tagVisibiility == Visible.ALWAYS ? visibleToClient : tagVisibiility == Visible.HIDE_FOR_OTHER_TEAMS ? playerTeam == null ? visibleToClient : entityTeam.isAlliedTo(
                playerTeam) && (entityTeam.canSeeFriendlyInvisibles() || visibleToClient) : tagVisibiility == Visible.HIDE_FOR_OWN_TEAM && (playerTeam == null ? visibleToClient : !entityTeam.isAlliedTo(
    ...
    public int getPackedOverlay(T animatable, float u) {
        ...
        return OverlayTexture.pack(OverlayTexture.u(u), OverlayTexture.v(
    ...
    public <H extends Entity, M extends MobEntity> void renderLeash(M mob, float partialTick, MatrixStack poseStack, IRenderTypeBuffer bufferSource, H leashHolder) {
        double lerpBodyAngle = (MathHelper.lerp(partialTick, mob.yBodyRotO,
                mob.yBodyRot) * ((float) Math.PI / 180)) + 1.5707963267948966;
