 ...
 * <pre>{@code
 *
 * public static final Holder<MobEffect> TEST_EFFECT = CommonStatusEffectRegistryInterface.registerStatusEffect(
 *     "modid",
 *     "effectName",
 *     () -> new CustomMobEffect(
 ...
 * </p>
 * <ul>
 * <li><code>registerStatusEffect</code> is a method to register a new status effect with the specified mod ID and
 * effect name.</li>
 ...
 * </p>
 * <p>
 * The {@link net.minecraft.world.effect.MobEffectCategory MobEffectCategory} class represents the category of a status
 * effect in the game.
 * </p>
 */
public interface CommonStatusEffectRegistryInterface {
     ...
     * @param <T>          The type of the status effect extending from {@link net.minecraft.world.effect.MobEffect}.
     * @return A holder that provides the registered status effect.
     */
    static <T extends MobEffect> Holder<T> registerStatusEffect(
        String modID,
        String effectName,
        Supplier<T> statusEffect
    ) {
        return Services.COMMON_REGISTRY.registerStatusEffect(modID, effectName, statusEffect);
    }
}
